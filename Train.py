...
if __name__ == '__main__':
    ...
    parser.add_argument("--style_weight", type=float, default=0.2,           # default = 0.02
    ...
    ######################### ADDED #########################
    parser.add_argument("--histogram_layers", default="r11,r41",
                        help='layers for style')
    parser.add_argument("--histogram_weight", type=float, default=0.5,
                        help='histogram loss weight')
    parser.add_argument("--histogram_bins", type=int, default=256,
                        help='histogram loss weight')
    parser.add_argument("--tv_layers", default="r11",
                        help='layers for total variation')
    parser.add_argument("--tv_weight", type=float, default=0.1,            # default = 0.01
                        help='total variational loss weight')
    ######################### ADDED #########################
    ...
    ######################### ADDED #########################
    opt.histogram_layers = opt.histogram_layers.split(',')
    opt.tv_layers = opt.tv_layers.split(',')
    ######################### ADDED #########################
    ...
    criterion = LossCriterion(opt.style_layers,
                              opt.content_layers,
                              opt.histogram_layers,
                              opt.style_weight,
                              opt.content_weight,
                              opt.histogram_weight,
                              opt.histogram_bins,
                              opt.tv_layers,
                              opt.tv_weight)
    ...
    for iteration in range(1,opt.niter+1):
        ...
        tF = vgg5(transfer)
        loss, styleLoss, contentLoss, histogramLoss, tvLoss = criterion(tF, sF_loss, cF_loss)

        # backward & optimization
        loss.backward()
        optimizer.step()
        print('Iteration: [%d/%d] Loss: %.4f contentLoss: %.4f styleLoss: %.4f tvLoss: %.4f Learng Rate is %.6f'%
             (opt.niter, iteration, loss, contentLoss, styleLoss, tvLoss, optimizer.param_groups[0]['lr']))
